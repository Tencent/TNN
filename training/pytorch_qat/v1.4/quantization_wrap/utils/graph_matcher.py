...
class OpTypePattern(Pattern):
  """A tree pattern that matches expressions with certain op types."""
  ...
  def match(self, op, tensor):
    ...
    if self._op_type != '*':
      ...
      # modify
      if str(op.kind()) not in self._op_type.split('|'):
        return None
    ...
    if len(list(op.inputs())) != len(self._inputs):
      return None
    ...
    for input_patterns in input_patterns_list:
      match_failed = False
      for input, input_pattern in zip(list(op.inputs()), input_patterns):
        input_match_result = input_pattern.match(input.node(), tensor=None)
...
class GraphMatcher(object):
  ...
  def _match_pattern(self, pattern, op, tensor):
    """Returns whether an expression rooted at `op` matches `pattern`.
    ...
    Args:
      pattern: An `Pattern`.
      op: A `Operation` to match against the pattern.
      tensor: the output Tensor of `op` that is used by the matching op of
        `pattern`'s parent. Can be None if `pattern` is already the root of the
        pattern tree.

    Returns:
      True if an expression rooted at `op` matches `pattern`.
    """
  ...
  def match_op(self, op):
    ...
    Args:
      op: `Operation` to match against the pattern.
  ...
  def match_ops(self, ops):
    ...
    Args:
      ops: collection of `Operation` to match against the pattern.

    Yields:
      `MatchResult` for each `Operation` that matches the pattern.
    """
    for op in ops:
  ...
  def match_graph(self, graph):
    ...
    Args:
      graph: `Graph` containing operations to match.

    Yields:
      `MatchResult` for each `Operation` in `graph` that matches the pattern.
    """
    for match_result in self.match_ops(graph.nodes()):
      yield match_result
