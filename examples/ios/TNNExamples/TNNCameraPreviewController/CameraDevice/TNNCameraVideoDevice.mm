...
- (id)initWithPreviewView:(UIView *)view
{
    self = [super init];
    if (self) {
        _cameraQueue = dispatch_queue_create("camera.queue", NULL);
...
- (void)setupCamera:(AVCaptureSessionPreset)sessionPreset
    ...
    dispatch_async(_cameraQueue, ^{
        auto success = [self setupCaptureSession:sessionPreset];
...
- (BOOL)setupCaptureSession:(AVCaptureSessionPreset)sessionPreset {
    ...
    for (AVCaptureDevice *dev in array) {
        if (dev.position == AVCaptureDevicePositionFront) _captureDevice = dev;
    }
    if (!_captureDevice) {
