...
namespace jit {
    void removeUselessOps(Block* block) {
        for (auto it = block->nodes().begin(), end = block->nodes().end(); it != end; ++it) {
            ...
            if (it->kind().toUnqualString() == std::string("data") || it->kind() == prim::NumToTensor ||
                it->kind() == aten::ScalarImplicit || it->kind() == aten::contiguous || it->kind() == aten::dropout ||
                it->kind() == aten::dropout_ || it->kind() == aten::feature_dropout || it->kind() == aten::clone) {
            ...
            if (it->kind() == aten::detach || it->kind() == aten::list ||
            ...
            if (it->kind() == aten::slice) {
                ...
                if (toIValue(start) && TNN_CONVERTER::GetValue<int64_t>(start) == 0 && toIValue(end) &&
                    TNN_CONVERTER::GetValue<int64_t>(end) == 9223372036854775807) {
                    if (it->inputs().size() > 4) {
                        auto stride = it->input(4);
                        if (toIValue(stride) && TNN_CONVERTER::GetValue<int64_t>(stride) != 1) {
                            continue;
                        }
                    }
                    it->output()->replaceAllUsesWith(it->input(0));
    ...
    */
    void FuseListStack(Block* block) {
        for (auto it = block->nodes().begin(); it != block->nodes().end();) {
            ...
            std::set<NodeKind> fusekind = {aten::stack};
    ...
    */
    void InputTypeInfer(Graph* graph) {
        ...
        static std::map<NodeKind, std::vector<ScalarType>> opInputTypes{
            // aten::embedding(Tensor weight, Tensor indices, int padding_idx, bool scale_grad_by_freq, bool sparse) ->
            // Tensor
            ...
            // aten::conv2d(Tensor input, Tensor weight, Tensor bias, int[] stride, int[] padding, int[] dilation, int
            // groups) -> Tensor
    ...
    void overloadDistinguish(Block* block) {
        ...
        for (auto it = block->nodes().begin(); it != block->nodes().end();) {
            ...
            switch (node->kind()) {
                ...
                case aten::max:
                case aten::sum:
                    if (node->inputs().size() > 1 && (node->input(1)->type()->kind() == c10::TypeKind::IntType ||
