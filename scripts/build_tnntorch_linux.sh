...
cp _pytnn.*.so ${TNN_INSTALL_DIR}/lib/ 
cp test/TNNTest ${TNN_INSTALL_DIR}/lib/

# deps
cp /usr/local/cuda/lib64/libcudart.so* ${TNN_INSTALL_DIR}/lib/
cp /usr/local/cuda/targets/x86_64-linux/lib/libnvrtc.so* ${TNN_INSTALL_DIR}/lib/
cp /usr/local/cuda/targets/x86_64-linux/lib/libnvToolsExt*.so* ${TNN_INSTALL_DIR}/lib/
cp /usr/lib/x86_64-linux-gnu/libcuda.so* ${TNN_INSTALL_DIR}/lib/
cp /usr/lib/x86_64-linux-gnu/libcublas*.so* ${TNN_INSTALL_DIR}/lib/
cp $TENSORRT_ROOT_DIR/lib/libnvinfer.so* ${TNN_INSTALL_DIR}/lib/
...
# torch
cp ${LIBTORCH_ROOT_DIR}/lib/libtorch.so ${TNN_INSTALL_DIR}/lib/
cp ${LIBTORCH_ROOT_DIR}/lib/libtorch_cpu.so ${TNN_INSTALL_DIR}/lib/
cp ${LIBTORCH_ROOT_DIR}/lib/libtorch_cuda.so ${TNN_INSTALL_DIR}/lib/
cp ${LIBTORCH_ROOT_DIR}/lib/libc10.so ${TNN_INSTALL_DIR}/lib/
cp ${LIBTORCH_ROOT_DIR}/lib/libc10_cuda.so ${TNN_INSTALL_DIR}/lib/
cp ${LIBTORCH_ROOT_DIR}/lib/libgomp*.so* ${TNN_INSTALL_DIR}/lib/
cp ${LIBTORCH_ROOT_DIR}/lib/libnvToolsExt*.so* ${TNN_INSTALL_DIR}/lib/
